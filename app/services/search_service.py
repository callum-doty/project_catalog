...
"""
Centralized search service that implements different search strategies
"""
...
from app.models.models import Document, LLMAnalysis, ExtractedText, DesignElement
from app.models.keyword_models import KeywordTaxonomy, KeywordSynonym, DocumentKeyword
from app.constants import CACHE_TIMEOUTS, DEFAULTS, SEARCH_TYPES, DOCUMENT_STATUSES
...
class SearchService:
    ...
    def search(self, query: str, **kwargs) -> Tuple[List[Dict], Dict, float]:
        ...
        try:
            ...
            if document_ids:
                ...
                all_keywords = self.get_document_hierarchical_keywords_bulk(document_ids)
                
                # Format documents for display
                formatted_documents = self._format_documents_for_display(documents, all_keywords)
            ...
            taxonomy_facets = self.generate_taxonomy_facets(primary_category, subcategory, specific_term)
            
            # Calculate response time
            response_time = (time.time() - start_time) * 1000
            
            return formatted_documents, pagination, taxonomy_facets, expanded_query, response_time
    ...
    def perform_keyword_search(self, query: str, expanded_query: Optional[Union[str, Set[str]]] = None):
        """
        ...
        try:
            ...
            if query:
                ...
                try:
                    ...
                    try:
                        # Get embeddings
                        query_embeddings = loop.run_until_complete(self.embeddings_service.generate_query_embeddings(query))
                    finally:
                    ...
                    if query_embeddings:
                        ...
                        from app.utils.query_builders import search_document_ids_by_vector
    ...
    def perform_vector_search(self, query: str):
        ...
        try:
            ...
            if not hasattr(Document, 'embeddings'):
                self.logger.warning("Vector search not available - Document model doesn't have embeddings attribute")
            ...
            if not query_embeddings:
                ...
                self.logger.warning("Vector embeddings generation failed, falling back to keyword search")
                return self.perform_keyword_search(query, set([query]))
    ...
    def perform_hybrid_search(self, query: str, expanded_query: Optional[Union[str, Set[str]]] = None):
        """
    ...
    def _apply_filters(self, query, **filters):
        ...
        if filter_type:
            query = query.join(LLMAnalysis, Document.id == LLMAnalysis.document_id, isouter=True)
            if ',' in filter_type:
        ...
        for doc in documents:
            try:
                ...
                try:
                    ...
                    self.logger.error(f"Preview generation failed for {doc.filename}: {str(e)}")
    ...
    def record_search_feedback(self, data):
        ...
        try:
            ...
            from app.models.keyword_models import SearchFeedback
