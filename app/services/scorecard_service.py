...
class ScorecardService:
    ...
    def create_scorecard(document_id):
        ...
        try:
            ...
            existing = DocumentScorecard.query.filter_by(document_id=document_id).first()
            if existing:
                logger.info(f"Scorecard already exists for document {document_id}")
                return existing
            ...
            db.session.rollback()
            logger.error(f"Error creating scorecard for document {document_id}: {str(e)}")
            raise
    ...
    def update_batch1_scores(document_id, metadata_score, text_score, flags=None):
        ...
        try:
            scorecard = DocumentScorecard.query.filter_by(document_id=document_id).first()
            if not scorecard:
            ...
            db.session.commit()
            logger.info(f"Updated Batch 1 scores for document {document_id}: metadata={metadata_score}, text={text_score}")
            
            return scorecard
        except Exception as e:
            db.session.rollback()
            logger.error(f"Error updating Batch 1 scores for document {document_id}: {str(e)}")
            raise
