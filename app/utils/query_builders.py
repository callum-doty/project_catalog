...
from app.models.keyword_models import DocumentKeyword, KeywordTaxonomy, KeywordSynonym
...
def filter_by_location(query, location=None):
    ...
    query = query.join(
        ...
        func.lower(DesignElement.geographic_location).like(f"%{location.lower()}%")
    )
...
def get_stuck_documents_query(hours=1):
    ...
    return Document.query.filter(
        Document.status.in_([DOCUMENT_STATUSES['PENDING'], DOCUMENT_STATUSES['PROCESSING']]),
...
def get_document_statistics(days=30):
    ...
    completed = base_query.filter(Document.status == DOCUMENT_STATUSES['COMPLETED']).count()
    failed = base_query.filter(Document.status == DOCUMENT_STATUSES['FAILED']).count()
    pending = base_query.filter(Document.status == DOCUMENT_STATUSES['PENDING']).count()
    processing = base_query.filter(Document.status == DOCUMENT_STATUSES['PROCESSING']).count()
    ...
    daily_processing = db.session.query(
        ...
        func.sum(case((Document.status == DOCUMENT_STATUSES['COMPLETED'], 1), else_=0)).label('completed'),
        func.sum(case((Document.status == DOCUMENT_STATUSES['FAILED'], 1), else_=0)).label('failed')
    ).filter(
