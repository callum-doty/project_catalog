...
import logging
from catalog.models import KeywordTaxonomy, KeywordSynonym, DocumentKeyword
...
class LLMResponseParser:
    ...
    def validate_confidence(value: float) -> float:
        ...
        try:
            ...
            logger.warning(
                f"Invalid confidence value: {value}, defaulting to 0.0")
            return 0.0
    ...
    def process_document_analysis(data: Dict[str, Any], document_id: int) -> bool:
        ...
        try:
            ...
            from catalog.models import LLMAnalysis
                    ...
                    if hasattr(parser, method_name):
                        ...
                        if component in component_to_model:
                            ...
                            model_class = getattr(import_module(
                                'app.models.models'), component_to_model[component])
    ...
    def parse_keywords(data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for keyword in keywords[:10]:  # Limit to 10 keywords max
                try:
                    keyword_text = LLMResponseParser.ensure_string(
                        keyword.get('text', ''))
                    if not keyword_text:
                    ...
                    logger.error(
                        f"Error parsing individual keyword {keyword}: {str(e)}")
                    continue
